...
% $Revision: 1.47 $  $Date: 2016/04/11 11:07:28 $
            ...
            elseif ishghandle(container) % && ~isa(container,'java.awt.Container')
                ...
                elseif ~isa(origContainer, 'figure') && ~isempty(hFig) && ~isa(origContainer, 'matlab.ui.Figure')
                    ...
                    if nargin==1 && nargout==1
                        try
                            try
                                handles = container.JavaFrame.getGUIDEView;  % for an HG2 matlab.ui.container.Panel
                            catch
                                handles = findjobj_fast(container);
                            end
                            if ~isempty(handles)
                                handles = handle(handles,'callbackproperties');
                                return
                            end
                        catch
...
function jControl = findjobj_fast(hControl, jContainer)
    ...
    if isempty(jContainer)
        ...
        jContainer = hFig.JavaFrame.getFigurePanelContainer.getComponent(0);
        warning(oldWarn);
    end
    jControl = [];
    counter = 100;
...
function jControl = findTooltipIn(jContainer)
    jControl = [];
    try
        tooltipStr = char(jContainer.getToolTipText);
        if strcmp(tooltipStr,'!@#$%^&*')
            jControl = jContainer;
            return;
        end
        for idx = jContainer.getComponentCount : -1 : 1
